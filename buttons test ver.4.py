...
# Функция для копирования зашифрованного сообщения
def copy_ciphertext():
...
# Функция для вставки текста из буфера обмена в поле ввода зашифрованного сообщения
def paste_ciphertext():
    clipboard_data = root.clipboard_get()
    ciphertext_entry.delete(0, tk.END)
    ciphertext_entry.insert(0, clipboard_data)

# Функция для создания контекстного меню
def create_context_menu(event):
    menu = tk.Menu(root, tearoff=0)
    menu.add_command(label="Копировать", command=copy_ciphertext)
    menu.add_command(label="Вставить", command=paste_ciphertext)
...
paste_button = ttk.Button(root, text="Вставить из буфера обмена", command=paste_ciphertext)
paste_button.pack()
